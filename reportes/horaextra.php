...
class PDF extends FPDF
{
	...
	function Footer()
	{
		...
		$this->Cell(80,7, "Encargado de Area",1,0,'C');
		$this->SetFont('Arial','B',7.5);
		$this->Cell(80,7, Manejador::obtenerDepartamentoN(5),1,0,'C');
		$this->Ln();
	...
	function Body($tabla, $fecha)
	{
		...
		foreach($tabla as $row)
		{
			...
			else
			{
				...
				if (Manejador::esMultidepartamental($id, $fecha) && Manejador::obtenerAcumuladoHE($id, $fecha) > Manejador::obtenerPorciento($id)) {
					$solicitantes = Manejador::esMultidepartamental($id, $fecha, 1);
					$porciento = Manejador::obtenerPorciento($id);
					$pagoTotal = $porciento/$solicitantes;
					$notacion = '**';
					...
					$this->Cell($w[3],8,utf8_decode($columna[3]),1,0,'C');
					$this->SetFont('Times','B',8);
