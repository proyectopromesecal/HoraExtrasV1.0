...
if($s->verificar())
{
	...
	{
		$_SESSION['rutaActual']="Pagos > Transporte";
		#echo '<pre>';
		#print_r($_POST);
		#echo '</pre>';
...
if($_POST)
{
	...
	if(isset($_POST['btnBuscar']))
	{
		...
		{
			$rsCustom = ManejadorTransporte::solicitudesAprobadas($_SESSION['dpto'], "",$_POST['slcFecha']);
		}
	...
	else if (isset($_POST['btnPagar']))
	{
		if(isset($_POST['check']))
		{
			...
			else
			{
				...
				{
					$idSl = $valor;
					ManejadorTransporte::pagarSolicitud($idSl);
				}			
			}
		}
		else
		...
		<div id='page'>
			<center></br>
				<fieldset style="width:70%;border-radius:8px;"><br>
					<form method="post" action="solicitudesTRPagadas.php">
						<div style="width:100%;">
						...
						<div class='center' style="height:50%;overflow:auto;">
							<table class='tab_cadre_fixe' style="width:100%;">
								...
								<?php 
									...
									{
										$rs =ManejadorTransporte::solicitudesAprobadas("","","");
									}
									if($rs)
									{
										while($fila=sqlsrv_fetch_array($rs, SQLSRV_FETCH_ASSOC))
										{
											$n = ManejadorTransporte::obtenerNombre($fila['id']);
											$estado = ManejadorTransporte::verEstado($fila['id']);
											$pagado = ManejadorTransporte::verificarPagado($fila['id']);
											echo "
												<tr class='tab_bg_2'>
